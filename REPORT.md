...
Результат: RSS снижен с 13.5 GB до 3.0 GB, decode ускорен с 3.5 до 9.1 tok/s
(за счёт выбора модели + привязки к big cores). NPU compute = 80% bottleneck
для dense 8B, дальнейшая оптимизация кода невозможна — ограничение аппаратное.

ЛУЧШИЕ РЕЗУЛЬТАТЫ (big cores only, taskset 0xf0 -t 4):
  - Gemma-4 E2B Q8_0: 9.1 tok/s, качество 5/5, RSS 4.7 GB  ← ЛУЧШИЙ DENSE
  - Qwen2.5-Coder-3B Q8_0: 8.1 tok/s, качество 4/5, RSS 3.0 GB  ← ЛУЧШИЙ ПО RSS
  - LFM2.5-8B-A1B Q8_0: 10 tok/s, MoE, качество OK, RSS 13.3 GB  ← ЛУЧШИЙ ПО СКОРОСТИ
  - T-lite 8B Q8_0: 4.1 tok/s, качество 5/5, RSS 7.5 GB  ← BASELINE
...
| Decode        | 3.5 tok/s  | 3.5 tok/s  | без измен.    |
| Качество      | 5/5        | 5/5        | OK            |
...
- CPU: taskset 0xf0 -t 4 (только big cores A76 @ 2.3 GHz)

**ВАЖНО:** RK3588 — big.LITTLE (4×A76 + 4×A55). LITTLE ядра замедляют
NPU-инференс на 17-33%. Все результаты ниже — на big cores.

### 4.2 Dense модели на NPU (big cores only)
...
### 4.3 Влияние big.LITTLE на скорость

| Модель                 | 8 ядер (big+LITTLE) | 4 big      | Прирост |
|------------------------|----------------------|------------|---------|
...
Причина: LITTLE ядра (A55 @ 1.8 GHz) медленнее выполняют CPU-side работу
(dequantize, quantize, pack, layernorm). С 8 потоками scheduler
назначает часть работы на LITTLE, создавая bottleneck.
Решение: taskset 0xf0 + -t 4 для привязки только к big cores (A76).

### 4.4 MoE модель (CPU-only)

| Модель         | Параметры       | Квант | RSS    | NPU buf | Decode   | Качество |
|----------------|-----------------|-------|--------|---------|----------|----------|
| LFM2.5-8B-A1B | 8B MoE (1B act) | Q8_0  | 13.3 GB| 0.7 GB  | 10 tok/s | OK+CoT   |

### 4.5 Ключевые наблюдения
...
5.6 Сравнение с оригинальным rk-llama.cpp (upstream)

  Бенчмарк: оригинал (github.com/invisiofficial/rk-llama.cpp) vs оптимизированный,
  обе сборки на big cores (taskset 0xf0 -t 4).

  | Модель                 | Оригинал tok/s | Оптимизир. tok/s | Оригинал RSS | Оптимизир. RSS | RSS экономия |
  |------------------------|----------------|------------------|-------------|----------------|--------------|
  ...
  Вывод: скорость decode без изменений (NPU compute = 80% bottleneck, не устраним
  программно). Основной эффект оптимизаций — снижение RSS на 2-6 GB (madvise
  освобождает GGUF mmap страницы после NPU DMA). Для моделей с большим NPU buffer
  эффект значительнее (47-49% экономия RSS).

---

## 6. Файлы

Исходный код оптимизаций: [github.com/Sinclear/rk3588-llama-npu](https://github.com/Sinclear/rk3588-llama-npu)

- `ggml-rknpu2.cpp` — основной backend (madvise + tiling cache v5 + c_bound)
- `tiling_cache.h/cpp` — кэш тилинга (v5, C + c_bound)
- `rknpu2-configuration.h/cpp` — конфигурация RKNN (включая Gemma-4 Q8_0)
- `ggml-rknpu2.patch` — diff против upstream rk-llama.cpp

Протестированные модели (HuggingFace, unsloth):
...
По скорости (decode, big cores only):
1. LFM2.5-8B-A1B Q8_0: 10 tok/s (MoE, CPU-only, RSS 13.3 GB)
2. Gemma-4 E2B Q8_0: 9.1 tok/s (dense, NPU, RSS 4.7 GB, качество 5/5)
3. Qwen2.5-Coder-3B Q8_0: 8.1 tok/s (dense, NPU, RSS 3.0 GB, качество 4/5)
...
**Рекомендация: Gemma-4 E2B Q8_0 + taskset 0xf0 -t 4 — лучший баланс**
...
Обязательные параметры для максимальной производительности:
...
- `taskset 0xf0` + `-t 4`: привязка к big cores (A76), прирост +17-33%
- `ulimit -n 65536`: достаточно file descriptors для NPU contexts
- `-ngl 99`: все слои на NPU (W8A8 pipeline)
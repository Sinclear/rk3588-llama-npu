...
Rknpu2ConfigManager::Rknpu2ConfigManager() {
    ...
    // Reading NPU disable flag
    bool npu_disabled = false;
    const char* env_disable = std::getenv("RKNPU_DISABLE");
    if (env_disable != nullptr && std::string(env_disable) == "1") {
        npu_disabled = true;
    }

    // Reading max NPU buffer size in MiB (0 = unlimited)
    size_t max_buffer_mb = 0;
    const char* env_max_buffer = std::getenv("RKNPU_MAX_BUFFER_MB");
    if (env_max_buffer != nullptr) {
        max_buffer_mb = (size_t)std::strtoul(env_max_buffer, nullptr, 10);
    }
    ...
    rk3588_config.npu_disabled = npu_disabled;
    rk3588_config.max_buffer_mb = max_buffer_mb;

...
struct Rknpu2DeviceConfig {
    ...
    // NPU disable flag (RKNPU_DISABLE=1)
    bool npu_disabled = false;

    // Maximum NPU buffer size in MiB (RKNPU_MAX_BUFFER_MB=0 means unlimited)
    ...
    // Running NPU buffer accumulator for RKNPU_MAX_BUFFER_MB enforcement
    mutable std::shared_ptr<std::mutex> buffer_limit_mutex = std::make_shared<std::mutex>();
    mutable size_t running_npu_buffer_size = 0;
